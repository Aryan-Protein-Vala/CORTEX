...
import { ArrowRight, Check, Menu, Moon, Network, Plus, Sun, X } from 'lucide-react'
...
const faqs = [
  ...
  ['Where is my data? Are you stealing my shit?', 'Relax, we don\'t want your garbage data. Start locally, use Cortex Cloud, or self-host if you’re a paranoid gigachad.'],
...
export default function Page() {
  ...
  return <main>
    <header className="nav"><a href="#top" className="logo">Cortex<span>.</span></a><nav className="nav-links"><a href="#how">How it works</a><a href="#specs">Specs</a><a href="#pricing">Pricing</a><a href="https://github.com/Aryan-Protein-Vala/CORTEX" target="_blank" rel="noreferrer">GitHub</a></nav><div className="nav-actions"><ThemeToggle /><a href="#pricing" className="button small">Start free <ArrowRight size={15}/></a><button className="menu-button" onClick={() => setMenu(!menu)} aria-label="Toggle menu">{menu ? <X/> : <Menu/>}</button></div></header>
    <AnimatePresence>{menu && <motion.div className="mobile-menu" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}><a href="#how" onClick={() => setMenu(false)}>How it works</a><a href="#specs" onClick={() => setMenu(false)}>Specs</a><a href="#pricing" onClick={() => setMenu(false)}>Pricing</a><a href="#faq" onClick={() => setMenu(false)}>FAQ</a></motion.div>}</AnimatePresence>
    <section id="top" className="hero wrap"><div className="hero-copy"><p className="eyebrow">MEMORY INFRASTRUCTURE / 01</p><h1>AI is fucking stupid.<br/><em>We fixed it.</em></h1><p className="hero-sub">I was so goddamn frustrated with AI amnesia that I built this. Stop paying out the ass for massive context windows that hallucinate anyway. Give every AI model a single Hive Mind brain that remembers your shit permanently.</p><div className="hero-actions"><a className="button" href="#pricing">Fuck OpenAI's token fees <ArrowRight size={17}/></a><a className="button ghost" href="#demo">See this shit work <ArrowRight size={17}/></a></div><p className="micro">Open protocol · Rust core · Built out of pure rage</p></div><div className="hero-visual"><div className="visual-label"><Network size={15}/> LIVE HIVE MIND GRAPH <span>●</span></div><Graph active={7}/><div className="visual-caption"><span>context_window</span><strong>RIP</strong><small>persistent / Big Tech is cooked</small></div></div></section>
    ...
    <section className="section muted proof"><div className="wrap"><Reveal><p className="eyebrow">THE REVOLUTION / 06</p><h2>They are <em>terrified.</em></h2></Reveal><div className="metrics"><div><strong>9.9k</strong><span>GitHub stars from gigachads</span></div><div><strong>∞</strong><span>Saved in API costs</span></div><div><strong>0ms</strong><span>Tolerance for amnesia</span></div></div><p className="trust">Making devs at these companies cope hard: <span>OpenAI</span><span>Anthropic</span><span>Google</span><span>Meta</span></p></div></section>
    <section className="section wrap"><div className="split"><Reveal className="split-text"><p className="eyebrow">THE MATH / 06</p><h2>The more you yap,<br/><em>the more you save.</em></h2><p>Context windows are a mathematical scam. They scale O(N). If you talk to ChatGPT 100 times, you pay to re-send the first 99 messages on the 100th turn. It's fucking ridiculous.</p><p>Cortex scales O(1). We extract the Semantic Triplets, store them in the graph, and only inject the exact 500-token `[SYSTEM CORTEX CONTEXT]` you need. Your token usage drops dramatically over time.</p></Reveal><Reveal className="split-visual"><div style={{display: 'flex', gap: '20px', height: '200px', alignItems: 'flex-end', justifyContent: 'center', background: 'var(--bg-card)', padding: '20px', borderRadius: '12px'}}><div style={{width: '80px', height: '100%', background: '#ff4545', borderRadius: '4px 4px 0 0', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '10px', fontWeight: 'bold'}}>OpenAI</div><div style={{width: '80px', height: '20%', background: 'var(--text)', color: 'var(--bg)', borderRadius: '4px 4px 0 0', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '10px', fontWeight: 'bold'}}>Cortex</div></div></Reveal></div></section>
    <section id="pricing" className="section wrap"><div className="pricing-head"><Reveal><p className="eyebrow">ACCESS / 07</p><h2>Stop being a <em>clown.</em></h2></Reveal><div className="billing"><button className={!annual ? 'selected' : ''} onClick={() => setAnnual(false)}>Monthly</button><button className={annual ? 'selected' : ''} onClick={() => setAnnual(true)}>Annual <span>Save bigly</span></button></div></div><div className="pricing-grid">{[['Broke','$0','For peasants',['Basic Graph DB','Local vectors','Skill issues']],['Chad',annual ? '$69/yr' : '$8/mo','For the elite devs',['Unlimited Hive Mind','No memory decay BS','Based support']],['Whale','Custom','For terrified platforms',['Steal our protocol','White-glove dev','Save your startup']]].map(([name,price,desc,features], i) => <article className={`price-card ${i === 1 ? 'featured' : ''}`} key={name}>{i === 1 && <span className="popular">LITERALLY FREE MONEY</span>}<p className="mono">{name.toUpperCase()}</p><h3>{price}</h3><p>{desc}</p><ul>{(features as string[]).map(f => <li key={f}><Check size={16}/>{f}</li>)}</ul><a href="#top" className={`button ${i === 1 ? '' : 'ghost'}`}>{i === 2 ? 'Beg sales' : i === 1 ? 'Become a Chad' : 'Stay broke'} <ArrowRight size={16}/></a></article>)}</div></section>
